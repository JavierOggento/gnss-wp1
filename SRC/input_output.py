...
def check_conf_param(key, fields, min_fields, max_fields, low_lim, upp_lim):
    ...
    if len_fields < min_fields:
        sys.stderr.write(f"ERROR: Too few fields ({len_fields}) for configuration parameter {key}. Minimum = {min_fields}\n")
        sys.exit(-1)

    if len_fields > max_fields:
        sys.stderr.write(f"ERROR: Too many fields ({len_fields}) for configuration parameter {key}. Maximum = {max_fields}\n")
        sys.exit(-1)
        ...
        if isinstance(low_lim[i], (int, float)):
            try:
                if field < low_lim[i] or field > upp_lim[i]:
                    sys.stderr.write(f"ERROR: Configuration parameter {key} {field} is out of range [{low_lim[i]}, {upp_lim[i]}]\n")
            except:
                sys.stderr.write(f"ERROR: Wrong type for configuration parameter {key}\n")
                sys.exit(-1)
    ...
    with open(cfg_file, 'r') as f:
        ...
        for line in lines:
            if line[0] != '#':
                ...
                if fields:
                    if len(fields) == 1:
                        sys.stderr.write(f"ERROR: Configuration file contains a parameter with no value: {line}")
                        sys.exit(-1)
                    
                    key = fields[0]
                    
                    if key == 'SAMPLING_RATE':
                        conf[key] = check_conf_param(key, fields, 1, 1, [0], [86400])
                        n_read_params += 1
                    
                    elif key == 'NAV_SOLUTION':
                        conf[key] = check_conf_param(key, fields, 1, 1, [None], [None])
                        n_read_params += 1
                    
                    elif key == 'GNSS_OUT':
                        conf[key] = check_conf_param(key, fields, 1, 1, [0], [1])
                        n_read_params += 1
                    
                    elif key == 'SEED':
                        conf[key] = check_conf_param(key, fields, 1, 1, [0.0], [np.inf])
                        n_read_params += 1
                        ...
                        conf[key] = check_conf_param(key, fields, 1, 1, [0], [90])
                        n_read_params += 1
                        ...
                        conf[key] = check_conf_param(key, fields, 2, 2, [None, None], [None, None])
                        n_read_params += 1
...
def read_traj_data(file_path):
    ...
    df = pd.read_fwf(file_path, widths=col_widths, names=column_names, skiprows=1)
...
def read_sp3(file_path):
    ...
    for line in lines:
        if line.startswith('#'):  # Header lines
            ...
            current_epoch = time_parts[3] * 3600 + time_parts[4] * 60 + time_parts[5]  # Convert to seconds of the day
        elif reading_ephemeris and (line.startswith('P') or line.startswith('V')):
            values = line.split()
            prn = values[0][1:]
            if prn.startswith('G') or prn.startswith('E'):  # Only include GPS (G) and Galileo (E)
                x, y, z, clock = map(float, values[1:])
                ephemeris_data.append([current_epoch, prn, x, y, z,])
    
    positions_df = pd.DataFrame(ephemeris_data, columns=['SOD', 'PRN', 'X', 'Y', 'Z',]).groupby('PRN')
